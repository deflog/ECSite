...
public class Mydata extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        request.setAttribute("error","セッションの有効期限が切れています");
        RequestDispatcher dispatch = request.getRequestDispatcher("/error.jsp");
        dispatch.forward(request, response);
        
        }
        UserDataDAO uda = new UserDataDAO();
        try{
        
        UserData ud = new UserData().Changeud(uda.SearchID((String)session.getAttribute("user")));
        //ud.Changeud(uda.SearchID((String)session.getAttribute("user")));
        session.setAttribute("ud", ud);
        String disp = "/Mydata.jsp";
